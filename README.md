...
**Idea clave:** el objeto final (`pago` decorado varias veces) sigue comportándose como un `Pago` normal; quien lo usa no necesita saber cuántas capas tiene.
...
## 6. Conceptos nuevos usados en este proyecto
 
- **Método `final`** (Plantilla de Método): impide que una subclase sobrescriba ese método; se usa para proteger el orden del algoritmo.
- **Método gancho (*hook method*)**: método con una implementación por defecto en la clase base, que las subclases pueden sobrescribir opcionalmente (no es obligatorio como un método abstracto).
- **Creación perezosa (*lazy initialization*)**: crear el objeto solo en el momento en que realmente se necesita, en lugar de crearlo por adelantado (usado en `Gestor.getInstancia()` del Singleton y en `Proxy` antes de instanciar `SolicitudAnulacion`).
- **Delegación**: cuando un objeto no resuelve una tarea por sí mismo, sino que se la pasa a otro objeto que tiene internamente (se ve en el Decorador y en el Proxy).
## 7. Resumen rápido de los 5 patrones
